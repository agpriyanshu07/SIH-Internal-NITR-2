import{a2 as _0,X as u2,a5 as W2}from"./index-HQDGQRPD.js";const y0=Math.PI,s0=y0*2,C2=y0/180,p3=1440,w2=398600.8,L0=6378.135,Z0=60/Math.sqrt(L0*L0*L0/w2),T2=L0*Z0/60,_2=1/Z0,$0=.001082616,$2=-253881e-11,k2=-165597e-11,k0=$2/$0,e2=2/3,P2=1440/(2*y0);function H2(S,O){const o=[31,S%4===0?29:28,31,30,31,30,31,31,30,31,30,31],h=Math.floor(O);let t=1,M=0;for(;h>M+o[t-1]&&t<12;)M+=o[t-1],t+=1;const R=t,i=h-M;let D=(O-h)*24;const d=Math.floor(D);D=(D-d)*60;const E=Math.floor(D),e=(D-E)*60;return{mon:R,day:i,hr:d,minute:E,sec:e}}function G2(S,O,o,h,t,M,R=0){return 367*S-Math.floor(7*(S+Math.floor((O+9)/12))*.25)+Math.floor(275*O/9)+o+17210135e-1+((R/6e4+M/60+t)/60+h)/24}function V2(S,O,o,h,t,M,R=0){if(S instanceof Date){const i=S;return G2(i.getUTCFullYear(),i.getUTCMonth()+1,i.getUTCDate(),i.getUTCHours(),i.getUTCMinutes(),i.getUTCSeconds(),i.getUTCMilliseconds())}return G2(S,O,o,h,t,M,R)}function n3(S,O){const o=S-24150195e-1,h=o/365.25;let t=1900+Math.floor(h),M=Math.floor((t-1901)*.25),R=o-((t-1900)*365+M)+1e-11;R<1&&(t-=1,M=Math.floor((t-1901)*.25),R=o-((t-1900)*365+M));const i=H2(t,R),{mon:D,day:d,hr:E,minute:e}=i,l=i.sec-864e-9;return new Date(Date.UTC(t,D-1,d,E,e,Math.floor(l)))}function Y2(S,O){const{e3:o,ee2:h,peo:t,pgho:M,pho:R,pinco:i,plo:D,se2:d,se3:E,sgh2:e,sgh3:l,sgh4:I,sh2:W,sh3:J,si2:g,si3:x,sl2:Q,sl3:K,sl4:H,t:A,xgh2:U,xgh3:Z,xgh4:u,xh2:F,xh3:B0,xi2:V,xi3:a,xl2:D0,xl3:w,xl4:t0,zmol:m,zmos:M0}=S,{init:T,opsmode:q}=O;let{ep:C0,inclp:_,nodep:P,argpp:$,mp:Y}=O,B,s,n0,j,a0,X,R0,S0,O0,r,G,U0,f,p,N,c,z,y,k,b,v;const d0=119459e-10,c0=.01675,e0=.00015835218,E0=.0549;v=M0+d0*A,T==="y"&&(v=M0),b=v+2*c0*Math.sin(v),z=Math.sin(b),r=.5*z*z-.25,G=-.5*z*Math.cos(b);const x0=d*r+E*G,z0=g*r+x*G,n=Q*r+K*G+H*z,m0=e*r+l*G+I*z,I0=W*r+J*G;v=m+e0*A,T==="y"&&(v=m),b=v+2*E0*Math.sin(v),z=Math.sin(b),r=.5*z*z-.25,G=-.5*z*Math.cos(b);const W0=h*r+o*G,u0=V*r+a*G,N0=D0*r+w*G+t0*z,l0=U*r+Z*G+u*z,X0=F*r+B0*G;return U0=x0+W0,N=z0+u0,c=n+N0,f=m0+l0,p=I0+X0,T==="n"&&(U0-=t,N-=i,c-=D,f-=M,p-=R,_+=N,C0+=U0,j=Math.sin(_),n0=Math.cos(_),_>=.2?(p/=j,f-=n0*p,$+=f,P+=p,Y+=c):(X=Math.sin(P),a0=Math.cos(P),B=j*X,s=j*a0,R0=p*a0+N*n0*X,S0=-p*X+N*n0*a0,B+=R0,s+=S0,P%=s0,P<0&&q==="a"&&(P+=s0),y=Y+$+n0*P,O0=c+f-N*P*j,y+=O0,k=P,P=Math.atan2(B,s),P<0&&q==="a"&&(P+=s0),Math.abs(k-P)>y0&&(P<k?P+=s0:P-=s0),Y+=c,$=y-Y-n0*P)),{ep:C0,inclp:_,nodep:P,argpp:$,mp:Y}}function S3(S){const{epoch:O,ep:o,argpp:h,tc:t,inclp:M,nodep:R,np:i}=S;let D,d,E,e,l,I,W,J,g,x,Q,K,H,A,U,Z,u,F,B0,V,a,D0,w,t0,m,M0,T,q,C0,_,P,$,Y,B,s,n0,j,a0,X,R0,S0,O0,r,G,U0,f,p,N,c,z,y,k,b,v,d0,c0,e0,E0,x0,z0,n,m0,I0;const W0=.01675,u0=.0549,N0=29864797e-13,l0=47968065e-14,X0=.39785416,K0=.91744867,i0=.1945905,q0=-.98088458,H0=i,p0=o,F0=Math.sin(R),b0=Math.cos(R),T0=Math.sin(h),G0=Math.cos(h),P0=Math.sin(M),C=Math.cos(M),h0=p0*p0,r0=1-h0,J0=Math.sqrt(r0),L=0,B2=0,n2=0,D2=0,t2=0,Q0=O+18261.5+t/1440,Y0=(4.523602-.00092422029*Q0)%s0,A0=Math.sin(Y0),V0=Math.cos(Y0),S2=.91375164-.03568096*V0,O2=Math.sqrt(1-S2*S2),f0=.089683511*A0/O2,g0=Math.sqrt(1-f0*f0),o0=5.8351514+.001944368*Q0;let j0=.39785416*A0/O2;const i2=g0*V0+.91744867*f0*A0;j0=Math.atan2(j0,i2),j0+=o0-Y0;const d2=Math.cos(j0),I2=Math.sin(j0);V=i0,a=q0,t0=K0,m=X0,D0=b0,w=F0,Q=N0;const A2=1/H0;let M2=0;for(;M2<2;)M2+=1,D=V*D0+a*t0*w,E=-a*D0+V*t0*w,W=-V*w+a*t0*D0,J=a*m,g=a*w+V*t0*D0,x=V*m,d=C*W+P0*J,e=C*g+P0*x,l=-P0*W+C*J,I=-P0*g+C*x,K=D*G0+d*T0,H=E*G0+e*T0,A=-D*T0+d*G0,U=-E*T0+e*G0,Z=l*T0,u=I*T0,F=l*G0,B0=I*G0,n=12*K*K-3*A*A,m0=24*K*H-6*A*U,I0=12*H*H-3*U*U,k=3*(D*D+d*d)+n*h0,b=6*(D*E+d*e)+m0*h0,v=3*(E*E+e*e)+I0*h0,d0=-6*D*l+h0*(-24*K*F-6*A*Z),c0=-6*(D*I+E*l)+h0*(-24*(H*F+K*B0)+-6*(A*u+U*Z)),e0=-6*E*I+h0*(-24*H*B0-6*U*u),E0=6*d*l+h0*(24*K*Z-6*A*F),x0=6*(e*l+d*I)+h0*(24*(H*Z+K*u)-6*(U*F+A*B0)),z0=6*e*I+h0*(24*H*u-6*U*B0),k=k+k+r0*n,b=b+b+r0*m0,v=v+v+r0*I0,p=Q*A2,f=-.5*p/J0,N=p*J0,U0=-15*p0*N,c=K*A+H*U,z=H*A+K*U,y=H*U-K*A,M2===1&&(M0=U0,T=f,q=p,C0=N,_=c,P=z,$=y,Y=k,B=b,s=v,n0=d0,j=c0,a0=e0,X=E0,R0=x0,S0=z0,O0=n,r=m0,G=I0,V=d2,a=I2,t0=S2,m=O2,D0=g0*b0+f0*F0,w=F0*g0-b0*f0,Q=l0);const E2=(4.7199672+(.2299715*Q0-o0))%s0,a2=(6.2565837+.017201977*Q0)%s0,m2=2*M0*P,v0=2*M0*$,w0=2*T*j,h2=2*T*(a0-n0),R2=-2*q*B,p2=-2*q*(s-Y),x2=-2*q*(-21-9*h0)*W0,U2=2*C0*r,r2=2*C0*(G-O0),f2=-18*C0*W0,o2=-2*T*R0,c2=-2*T*(S0-X),s2=2*U0*z,g2=2*U0*y,z2=2*f*c0,X2=2*f*(e0-d0),b2=-2*p*b,J2=-2*p*(v-k),Q2=-2*p*(-21-9*h0)*u0,Z2=2*N*m0,y2=2*N*(I0-n),q2=-18*N*u0,j2=-2*f*x0,v2=-2*f*(z0-E0);return{snodm:F0,cnodm:b0,sinim:P0,cosim:C,sinomm:T0,cosomm:G0,day:Q0,e3:g2,ee2:s2,em:p0,emsq:h0,gam:o0,peo:L,pgho:D2,pho:t2,pinco:B2,plo:n2,rtemsq:J0,se2:m2,se3:v0,sgh2:U2,sgh3:r2,sgh4:f2,sh2:o2,sh3:c2,si2:w0,si3:h2,sl2:R2,sl3:p2,sl4:x2,s1:U0,s2:f,s3:p,s4:N,s5:c,s6:z,s7:y,ss1:M0,ss2:T,ss3:q,ss4:C0,ss5:_,ss6:P,ss7:$,sz1:Y,sz2:B,sz3:s,sz11:n0,sz12:j,sz13:a0,sz21:X,sz22:R0,sz23:S0,sz31:O0,sz32:r,sz33:G,xgh2:Z2,xgh3:y2,xgh4:q2,xh2:j2,xh3:v2,xi2:z2,xi3:X2,xl2:b2,xl3:J2,xl4:Q2,nm:H0,z1:k,z2:b,z3:v,z11:d0,z12:c0,z13:e0,z21:E0,z22:x0,z23:z0,z31:n,z32:m0,z33:I0,zmol:E2,zmos:a2}}function O3(S){const{cosim:O,argpo:o,s1:h,s2:t,s3:M,s4:R,s5:i,sinim:D,ss1:d,ss2:E,ss3:e,ss4:l,ss5:I,sz1:W,sz3:J,sz11:g,sz13:x,sz21:Q,sz23:K,sz31:H,sz33:A,t:U,tc:Z,gsto:u,mo:F,mdot:B0,no:V,nodeo:a,nodedot:D0,xpidot:w,z1:t0,z3:m,z11:M0,z13:T,z21:q,z23:C0,z31:_,z33:P,ecco:$,eccsq:Y}=S;let{emsq:B,em:s,argpm:n0,inclm:j,mm:a0,nm:X,nodem:R0,irez:S0,atime:O0,d2201:r,d2211:G,d3210:U0,d3222:f,d4410:p,d4422:N,d5220:c,d5232:z,d5421:y,d5433:k,dedt:b,didt:v,dmdt:d0,dnodt:c0,domdt:e0,del1:E0,del2:x0,del3:z0,xfact:n,xlamo:m0,xli:I0,xni:W0}=S,u0,N0,l0,X0,K0,i0,q0,H0,p0,F0,b0,T0,G0,P0,C,h0,r0,J0,L,B2,n2,D2,t2,Q0,Y0,A0,V0,S2,O2,f0,g0,o0;const j0=17891679e-13,i2=21460748e-13,d2=22123015e-14,I2=17891679e-13,A2=73636953e-16,M2=21765803e-16,E2=.0043752690880113,a2=37393792e-14,m2=11428639e-14,v0=.00015835218,w0=119459e-10;S0=0,X<.0052359877&&X>.0034906585&&(S0=1),X>=.00826&&X<=.00924&&s>=.5&&(S0=2);const h2=d*w0*I,R2=E*w0*(g+x),p2=-w0*e*(W+J-14-6*B),x2=l*w0*(H+A-6);let U2=-w0*E*(Q+K);(j<.052359877||j>y0-.052359877)&&(U2=0),D!==0&&(U2/=D);const r2=x2-O*U2;b=h2+h*v0*i,v=R2+t*v0*(M0+T),d0=p2-v0*M*(t0+m-14-6*B);const f2=R*v0*(_+P-6);let o2=-v0*t*(q+C0);(j<.052359877||j>y0-.052359877)&&(o2=0),e0=r2+f2,c0=U2,D!==0&&(e0-=O/D*o2,c0+=o2/D);const c2=0,s2=(u+Z*E2)%s0;if(s+=b*U,j+=v*U,n0+=e0*U,R0+=c0*U,a0+=d0*U,S0!==0){if(f0=(X/Z0)**e2,S0===2){g0=O*O;const g2=s;s=$;const z2=B;B=Y,o0=s*B,P0=-.306-(s-.64)*.44,s<=.65?(C=3.616-13.247*s+16.29*B,r0=-19.302+117.39*s-228.419*B+156.591*o0,J0=-18.9068+109.7927*s-214.6334*B+146.5816*o0,L=-41.122+242.694*s-471.094*B+313.953*o0,B2=-146.407+841.88*s-1629.014*B+1083.435*o0,n2=-532.114+3017.977*s-5740.032*B+3708.276*o0):(C=-72.099+331.819*s-508.738*B+266.724*o0,r0=-346.844+1582.851*s-2415.925*B+1246.113*o0,J0=-342.585+1554.908*s-2366.899*B+1215.972*o0,L=-1052.797+4758.686*s-7193.992*B+3651.957*o0,B2=-3581.69+16178.11*s-24462.77*B+12422.52*o0,s>.715?n2=-5149.66+29936.92*s-54087.36*B+31324.56*o0:n2=1464.74-4664.75*s+3763.64*B),s<.7?(Q0=-919.2277+4988.61*s-9064.77*B+5542.21*o0,D2=-822.71072+4568.6173*s-8491.4146*B+5337.524*o0,t2=-853.666+4690.25*s-8624.77*B+5341.4*o0):(Q0=-37995.78+161616.52*s-229838.2*B+109377.94*o0,D2=-51752.104+218913.95*s-309468.16*B+146349.42*o0,t2=-40023.88+170470.89*s-242699.48*B+115605.82*o0),Y0=D*D,u0=.75*(1+2*O+g0),N0=1.5*Y0,X0=1.875*D*(1-2*O-3*g0),K0=-1.875*D*(1+2*O-3*g0),q0=35*Y0*u0,H0=39.375*Y0*Y0,p0=9.84375*D*(Y0*(1-2*O-5*g0)+.33333333*(-2+4*O+6*g0)),F0=D*(4.92187512*Y0*(-2-4*O+10*g0)+6.56250012*(1+2*O-3*g0)),b0=29.53125*D*(2-8*O+g0*(-12+8*O+10*g0)),T0=29.53125*D*(-2-8*O+g0*(12+8*O-10*g0)),S2=X*X,O2=f0*f0,V0=3*S2*O2,A0=V0*I2,r=A0*u0*P0,G=A0*N0*C,V0*=f0,A0=V0*a2,U0=A0*X0*r0,f=A0*K0*J0,V0*=f0,A0=2*V0*A2,p=A0*q0*L,N=A0*H0*B2,V0*=f0,A0=V0*m2,c=A0*p0*n2,z=A0*F0*t2,A0=2*V0*M2,y=A0*b0*D2,k=A0*T0*Q0,m0=(F+a+a-(s2+s2))%s0,n=B0+d0+2*(D0+c0-E2)-V,s=g2,B=z2}S0===1&&(G0=1+B*(-2.5+.8125*B),r0=1+2*B,h0=1+B*(-6+6.60937*B),u0=.75*(1+O)*(1+O),l0=.9375*D*D*(1+3*O)-.75*(1+O),i0=1+O,i0=1.875*i0*i0*i0,E0=3*X*X*f0*f0,x0=2*E0*u0*G0*j0,z0=3*E0*i0*h0*d2*f0,E0=E0*l0*r0*i2*f0,m0=(F+a+o-s2)%s0,n=B0+w+d0+e0+c0-(V+E2)),I0=m0,W0=V,O0=0,X=V+c2}return{em:s,argpm:n0,inclm:j,mm:a0,nm:X,nodem:R0,irez:S0,atime:O0,d2201:r,d2211:G,d3210:U0,d3222:f,d4410:p,d4422:N,d5220:c,d5232:z,d5421:y,d5433:k,dedt:b,didt:v,dmdt:d0,dndt:c2,dnodt:c0,domdt:e0,del1:E0,del2:x0,del3:z0,xfact:n,xlamo:m0,xli:I0,xni:W0}}function K2(S){const O=(S-2451545)/36525;let o=-62e-7*O*O*O+.093104*O*O+(876600*3600+8640184812866e-6)*O+67310.54841;return o=o*C2/240%s0,o<0&&(o+=s0),o}function U3(S,O,o,h,t,M,R){return S instanceof Date?K2(V2(S)):K2(S)}function B3(S){const{ecco:O,epoch:o,inclo:h,opsmode:t}=S;let{no:M}=S;const R=O*O,i=1-R,D=Math.sqrt(i),d=Math.cos(h),E=d*d,e=(Z0/M)**e2,l=.75*$0*(3*E-1)/(D*i);let I=l/(e*e);const W=e*(1-I*I-I*(1/3+134*I*I/81));I=l/(W*W),M/=1+I;const J=(Z0/M)**e2,g=Math.sin(h),x=J*i,Q=1-5*E,K=-Q-E-E,H=1/J,A=x*x,U=J*(1-O),Z="n";let u;if(t==="a"){const F=o-7305,B0=Math.floor(F+1e-8),V=F-B0,a=.017202791694070362,D0=1.7321343856509375,w=5075514194322695e-30,t0=a+s0;u=(D0+a*B0+t0*V+F*F*w)%s0,u<0&&(u+=s0)}else u=U3(o+24332815e-1);return{no:M,method:Z,ainv:H,ao:J,con41:K,con42:Q,cosio:d,cosio2:E,eccsq:R,omeosq:i,posq:A,rp:U,rteosq:D,sinio:g,gsto:u}}function D3(S){const{irez:O,d2201:o,d2211:h,d3210:t,d3222:M,d4410:R,d4422:i,d5220:D,d5232:d,d5421:E,d5433:e,dedt:l,del1:I,del2:W,del3:J,didt:g,dmdt:x,dnodt:Q,domdt:K,argpo:H,argpdot:A,t:U,tc:Z,gsto:u,xfact:F,xlamo:B0,no:V}=S;let{atime:a,em:D0,argpm:w,inclm:t0,xli:m,mm:M0,xni:T,nodem:q,nm:C0}=S;const _=.13130908,P=2.8843198,$=.37448087,Y=5.7686396,B=.95240898,s=1.8014998,n0=1.050833,j=4.4108898,a0=.0043752690880113,X=720,R0=-720,S0=259200;let O0,r,G,U0,f,p,N,c,z=0,y=0;const k=(u+Z*a0)%s0;if(D0+=l*U,t0+=g*U,w+=K*U,q+=Q*U,M0+=x*U,O!==0){(a===0||U*a<=0||Math.abs(U)<Math.abs(a))&&(a=0,T=V,m=B0),U>0?O0=X:O0=R0;let b=381;for(;b===381;)O!==2?(N=I*Math.sin(m-_)+W*Math.sin(2*(m-P))+J*Math.sin(3*(m-$)),f=T+F,p=I*Math.cos(m-_)+2*W*Math.cos(2*(m-P))+3*J*Math.cos(3*(m-$)),p*=f):(c=H+A*a,G=c+c,r=m+m,N=o*Math.sin(G+m-Y)+h*Math.sin(m-Y)+t*Math.sin(c+m-B)+M*Math.sin(-c+m-B)+R*Math.sin(G+r-s)+i*Math.sin(r-s)+D*Math.sin(c+m-n0)+d*Math.sin(-c+m-n0)+E*Math.sin(c+r-j)+e*Math.sin(-c+r-j),f=T+F,p=o*Math.cos(G+m-Y)+h*Math.cos(m-Y)+t*Math.cos(c+m-B)+M*Math.cos(-c+m-B)+D*Math.cos(c+m-n0)+d*Math.cos(-c+m-n0)+2*(R*Math.cos(G+r-s)+i*Math.cos(r-s)+E*Math.cos(c+r-j)+e*Math.cos(-c+r-j)),p*=f),Math.abs(U-a)>=X?b=381:(y=U-a,b=0),b===381&&(m+=f*O0+N*S0,T+=N*O0+p*S0,a+=O0);C0=T+N*y+p*y*y*.5,U0=m+f*y+N*y*y*.5,O!==1?(M0=U0-2*q+2*k,z=C0-V):(M0=U0-q-w+k,z=C0-V),C0=V+z}return{atime:a,em:D0,argpm:w,inclm:t0,xli:m,mm:M0,xni:T,nodem:q,dndt:z,nm:C0}}function t3(S,O){let o,h,t,M,R,i,D,d,E,e,l,I,W,J,g,x,Q,K,H,A,U,Z,u,F,B0,V,a;S.t=O,S.error=_0.None;const w=S.mo+S.mdot*S.t,t0=S.argpo+S.argpdot*S.t,m=S.nodeo+S.nodedot*S.t;E=t0,U=w;const M0=S.t*S.t;if(u=m+S.nodecf*M0,Q=1-S.cc1*S.t,K=S.bstar*S.cc4*S.t,H=S.t2cof*M0,S.isimp!==1){D=S.omgcof*S.t;const p0=1+S.eta*Math.cos(w);i=S.xmcof*(p0*p0*p0-S.delmo),x=D+i,U=w+x,E=t0-x,I=M0*S.t,W=I*S.t,Q=Q-S.d2*M0-S.d3*I-S.d4*W,K+=S.bstar*S.cc5*(Math.sin(U)-S.sinmao),H=H+S.t3cof*I+W*(S.t4cof+S.t*S.t5cof)}S.tempa=Q,Z=S.no;let T=S.ecco;if(A=S.inclo,S.method==="d"){J=S.t;const p0={irez:S.irez,d2201:S.d2201,d2211:S.d2211,d3210:S.d3210,d3222:S.d3222,d4410:S.d4410,d4422:S.d4422,d5220:S.d5220,d5232:S.d5232,d5421:S.d5421,d5433:S.d5433,dedt:S.dedt,del1:S.del1,del2:S.del2,del3:S.del3,didt:S.didt,dmdt:S.dmdt,dnodt:S.dnodt,domdt:S.domdt,argpo:S.argpo,argpdot:S.argpdot,t:S.t,tc:J,gsto:S.gsto,xfact:S.xfact,xlamo:S.xlamo,no:S.no,atime:S.atime,em:T,argpm:E,inclm:A,xli:S.xli,mm:U,xni:S.xni,nodem:u,nm:Z};({em:T,argpm:E,inclm:A,mm:U,nodem:u,nm:Z}=D3(p0))}if(Z<=0)return S.error=_0.MeanMotionBelowZero,null;const q=(Z0/Z)**e2*Q*Q;if(Z=Z0/q**1.5,T-=K,T>=1||T<-.001)return S.error=_0.MeanEccentricityOutOfRange,null;T<1e-6&&(T=1e-6),U+=S.no*H,B0=U+E+u,u%=s0,E%=s0,B0%=s0,U=(B0-E-u)%s0;const C0={am:q,em:T,im:A,Om:u,om:E,mm:U,nm:Z},_=Math.sin(A),P=Math.cos(A);let $=T;if(F=A,e=E,a=u,V=U,M=_,t=P,S.method==="d"){const p0={inclo:S.inclo,init:"n",ep:$,inclp:F,nodep:a,argpp:e,mp:V,opsmode:S.operationmode},F0=Y2(S,p0);if({ep:$,nodep:a,argpp:e,mp:V}=F0,F=F0.inclp,F<0&&(F=-F,a+=y0,e-=y0),$<0||$>1)return S.error=_0.PerturbedEccentricityOutOfRange,null}S.method==="d"&&(M=Math.sin(F),t=Math.cos(F),S.aycof=-.5*k0*M,Math.abs(t+1)>15e-13?S.xlcof=-.25*k0*M*(3+5*t)/(1+t):S.xlcof=-.25*k0*M*(3+5*t)/15e-13);const Y=$*Math.cos(e);x=1/(q*(1-$*$));const B=$*Math.sin(e)+x*S.aycof,n0=(V+e+a+x*S.xlcof*Y-a)%s0;d=n0,g=9999.9;let j=1;for(;Math.abs(g)>=1e-12&&j<=10;)h=Math.sin(d),o=Math.cos(d),g=1-o*Y-h*B,g=(n0-B*o+Y*h-d)/g,Math.abs(g)>=.95&&(g>0?g=.95:g=-.95),d+=g,j+=1;const a0=Y*o+B*h,X=Y*h-B*o,R0=Y*Y+B*B,S0=q*(1-R0);if(S0<0)return S.error=_0.SemiLatusRectumBelowZero,null;const O0=q*(1-a0),r=Math.sqrt(q)*X/O0,G=Math.sqrt(S0)/O0,U0=Math.sqrt(1-R0);x=X/(1+U0);const f=q/O0*(h-B-Y*x),p=q/O0*(o-Y+B*x);l=Math.atan2(f,p);const N=(p+p)*f,c=1-2*f*f;x=1/S0;const z=.5*$0*x,y=z*x;S.method==="d"&&(R=t*t,S.con41=3*R-1,S.x1mth2=1-R,S.x7thm1=7*R-1);const k=O0*(1-1.5*y*U0*S.con41)+.5*z*S.x1mth2*c;if(k<1)return S.error=_0.Decayed,null;l-=.25*y*S.x7thm1*N;const b=a+1.5*y*t*N,v=F+1.5*y*t*M*c,d0=r-Z*z*S.x1mth2*N/Z0,c0=G+Z*z*(S.x1mth2*c+1.5*S.con41)/Z0,e0=Math.sin(l),E0=Math.cos(l),x0=Math.sin(b),z0=Math.cos(b),n=Math.sin(v),m0=Math.cos(v),I0=-x0*m0,W0=z0*m0,u0=I0*e0+z0*E0,N0=W0*e0+x0*E0,l0=n*e0,X0=I0*E0-z0*e0,K0=W0*E0-x0*e0,i0=n*E0,q0={x:k*u0*L0,y:k*N0*L0,z:k*l0*L0},H0={x:(d0*u0+c0*X0)*T2,y:(d0*N0+c0*K0)*T2,z:(d0*l0+c0*i0)*T2};return{position:q0,velocity:H0,meanElements:C0}}function M3(S,O){const{opsmode:o,epoch:h,xbstar:t,xecco:M,xargpo:R,xinclo:i,xmo:D,xno:d,xnodeo:E}=O;let e,l,I,W,J,g,x,Q,K,H,A,U,Z,u,F,B0,V,a,D0,w,t0,m,M0,T,q,C0,_,P,$,Y,B,s,n0,j,a0,X,R0,S0,O0,r,G,U0,f,p,N,c,z,y,k,b,v,d0,c0,e0,E0,x0;const z0=15e-13,n=S;n.isimp=0,n.method="n",n.aycof=0,n.con41=0,n.cc1=0,n.cc4=0,n.cc5=0,n.d2=0,n.d3=0,n.d4=0,n.delmo=0,n.eta=0,n.argpdot=0,n.omgcof=0,n.sinmao=0,n.t=0,n.t2cof=0,n.t3cof=0,n.t4cof=0,n.t5cof=0,n.x1mth2=0,n.x7thm1=0,n.mdot=0,n.nodedot=0,n.xlcof=0,n.xmcof=0,n.nodecf=0,n.irez=0,n.d2201=0,n.d2211=0,n.d3210=0,n.d3222=0,n.d4410=0,n.d4422=0,n.d5220=0,n.d5232=0,n.d5421=0,n.d5433=0,n.dedt=0,n.del1=0,n.del2=0,n.del3=0,n.didt=0,n.dmdt=0,n.dnodt=0,n.domdt=0,n.e3=0,n.ee2=0,n.peo=0,n.pgho=0,n.pho=0,n.pinco=0,n.plo=0,n.se2=0,n.se3=0,n.sgh2=0,n.sgh3=0,n.sgh4=0,n.sh2=0,n.sh3=0,n.si2=0,n.si3=0,n.sl2=0,n.sl3=0,n.sl4=0,n.gsto=0,n.xfact=0,n.xgh2=0,n.xgh3=0,n.xgh4=0,n.xh2=0,n.xh3=0,n.xi2=0,n.xi3=0,n.xl2=0,n.xl3=0,n.xl4=0,n.xlamo=0,n.zmol=0,n.zmos=0,n.atime=0,n.xli=0,n.xni=0,n.bstar=t,n.ecco=M,n.argpo=R,n.inclo=i,n.mo=D,n.no=d,n.nokozai=d,n.nodeo=E,n.operationmode=o;const m0=78/L0+1,I0=42/L0,W0=I0*I0*I0*I0;n.init="y",n.t=0;const u0={ecco:n.ecco,epoch:h,inclo:n.inclo,no:n.no,method:n.method,opsmode:n.operationmode},N0=B3(u0),{ao:l0,con42:X0,cosio:K0,cosio2:i0,eccsq:q0,omeosq:H0,posq:p0,rp:F0,rteosq:b0,sinio:T0}=N0;if(n.no=N0.no,n.con41=N0.con41,n.gsto=N0.gsto,n.a=(n.no*_2)**(-2/3),n.alta=n.a*(1+n.ecco)-1,n.altp=n.a*(1-n.ecco)-1,n.error=0,H0>=0||n.no>=0){if(n.isimp=0,F0<220/L0+1&&(n.isimp=1),_=m0,t0=W0,a=(F0-1)*L0,a<156){_=a-78,a<98&&(_=20);const P0=(120-_)/L0;t0=P0*P0*P0*P0,_=_/L0+1}D0=1/p0,c=1/(l0-_),n.eta=l0*n.ecco*c,U=n.eta*n.eta,A=n.ecco*n.eta,w=Math.abs(1-U),g=t0*c**4,x=g/w**3.5,W=x*n.no*(l0*(1+1.5*U+A*(4+U))+.375*$0*c/w*n.con41*(8+3*U*(8+U))),n.cc1=n.bstar*W,J=0,n.ecco>1e-4&&(J=-2*g*c*k0*n.no*T0/n.ecco),n.x1mth2=1-i0,n.cc4=2*n.no*x*l0*H0*(n.eta*(2+.5*U)+n.ecco*(.5+2*U)-$0*c/(l0*w)*(-3*n.con41*(1-2*A+U*(1.5-.5*A))+.75*n.x1mth2*(2*U-A*(1+U))*Math.cos(2*n.argpo))),n.cc5=2*x*l0*H0*(1+2.75*(U+A)+A*U),Q=i0*i0,f=1.5*$0*D0*n.no,p=.5*f*$0*D0,N=-.46875*k2*D0*D0*n.no,n.mdot=n.no+.5*f*b0*n.con41+.0625*p*b0*(13-78*i0+137*Q),n.argpdot=-.5*f*X0+.0625*p*(7-114*i0+395*Q)+N*(3-36*i0+49*Q),y=-f*K0,n.nodedot=y+(.5*p*(4-19*i0)+2*N*(3-7*i0))*K0,z=n.argpdot+n.nodedot,n.omgcof=n.bstar*J*Math.cos(n.argpo),n.xmcof=0,n.ecco>1e-4&&(n.xmcof=-e2*g*n.bstar/A),n.nodecf=3.5*H0*y*n.cc1,n.t2cof=1.5*n.cc1,Math.abs(K0+1)>15e-13?n.xlcof=-.25*k0*T0*(3+5*K0)/(1+K0):n.xlcof=-.25*k0*T0*(3+5*K0)/z0,n.aycof=-.5*k0*T0;const G0=1+n.eta*Math.cos(n.mo);if(n.delmo=G0*G0*G0,n.sinmao=Math.sin(n.mo),n.x7thm1=7*i0-1,2*y0/n.no>=225){n.method="d",n.isimp=1,G=0,F=n.inclo;const P0={epoch:h,ep:n.ecco,argpp:n.argpo,tc:G,inclp:n.inclo,nodep:n.nodeo,np:n.no,e3:n.e3,ee2:n.ee2,peo:n.peo,pgho:n.pgho,pho:n.pho,pinco:n.pinco,plo:n.plo,se2:n.se2,se3:n.se3,sgh2:n.sgh2,sgh3:n.sgh3,sgh4:n.sgh4,sh2:n.sh2,sh3:n.sh3,si2:n.si2,si3:n.si3,sl2:n.sl2,sl3:n.sl3,sl4:n.sl4,xgh2:n.xgh2,xgh3:n.xgh3,xgh4:n.xgh4,xh2:n.xh2,xh3:n.xh3,xi2:n.xi2,xi3:n.xi3,xl2:n.xl2,xl3:n.xl3,xl4:n.xl4,zmol:n.zmol,zmos:n.zmos},C=S3(P0);n.e3=C.e3,n.ee2=C.ee2,n.peo=C.peo,n.pgho=C.pgho,n.pho=C.pho,n.pinco=C.pinco,n.plo=C.plo,n.se2=C.se2,n.se3=C.se3,n.sgh2=C.sgh2,n.sgh3=C.sgh3,n.sgh4=C.sgh4,n.sh2=C.sh2,n.sh3=C.sh3,n.si2=C.si2,n.si3=C.si3,n.sl2=C.sl2,n.sl3=C.sl3,n.sl4=C.sl4,{sinim:l,cosim:e,em:K,emsq:H,s1:m,s2:M0,s3:T,s4:q,s5:C0,ss1:P,ss2:$,ss3:Y,ss4:B,ss5:s,sz1:n0,sz3:j,sz11:a0,sz13:X,sz21:R0,sz23:S0,sz31:O0,sz33:r}=C,n.xgh2=C.xgh2,n.xgh3=C.xgh3,n.xgh4=C.xgh4,n.xh2=C.xh2,n.xh3=C.xh3,n.xi2=C.xi2,n.xi3=C.xi3,n.xl2=C.xl2,n.xl3=C.xl3,n.xl4=C.xl4,n.zmol=C.zmol,n.zmos=C.zmos,{nm:V,z1:k,z3:b,z11:v,z13:d0,z21:c0,z23:e0,z31:E0,z33:x0}=C;const h0={init:n.init,ep:n.ecco,inclp:n.inclo,nodep:n.nodeo,argpp:n.argpo,mp:n.mo,opsmode:n.operationmode},r0=Y2(n,h0);n.ecco=r0.ep,n.inclo=r0.inclp,n.nodeo=r0.nodep,n.argpo=r0.argpp,n.mo=r0.mp,Z=0,u=0,B0=0;const J0={cosim:e,emsq:H,argpo:n.argpo,s1:m,s2:M0,s3:T,s4:q,s5:C0,sinim:l,ss1:P,ss2:$,ss3:Y,ss4:B,ss5:s,sz1:n0,sz3:j,sz11:a0,sz13:X,sz21:R0,sz23:S0,sz31:O0,sz33:r,t:n.t,tc:G,gsto:n.gsto,mo:n.mo,mdot:n.mdot,no:n.no,nodeo:n.nodeo,nodedot:n.nodedot,xpidot:z,z1:k,z3:b,z11:v,z13:d0,z21:c0,z23:e0,z31:E0,z33:x0,ecco:n.ecco,eccsq:q0,em:K,argpm:Z,inclm:F,mm:B0,nm:V,nodem:u,irez:n.irez,atime:n.atime,d2201:n.d2201,d2211:n.d2211,d3210:n.d3210,d3222:n.d3222,d4410:n.d4410,d4422:n.d4422,d5220:n.d5220,d5232:n.d5232,d5421:n.d5421,d5433:n.d5433,dedt:n.dedt,didt:n.didt,dmdt:n.dmdt,dnodt:n.dnodt,domdt:n.domdt,del1:n.del1,del2:n.del2,del3:n.del3,xfact:n.xfact,xlamo:n.xlamo,xli:n.xli,xni:n.xni},L=O3(J0);n.irez=L.irez,n.atime=L.atime,n.d2201=L.d2201,n.d2211=L.d2211,n.d3210=L.d3210,n.d3222=L.d3222,n.d4410=L.d4410,n.d4422=L.d4422,n.d5220=L.d5220,n.d5232=L.d5232,n.d5421=L.d5421,n.d5433=L.d5433,n.dedt=L.dedt,n.didt=L.didt,n.dmdt=L.dmdt,n.dnodt=L.dnodt,n.domdt=L.domdt,n.del1=L.del1,n.del2=L.del2,n.del3=L.del3,n.xfact=L.xfact,n.xlamo=L.xlamo,n.xli=L.xli,n.xni=L.xni}n.isimp!==1&&(I=n.cc1*n.cc1,n.d2=4*l0*c*I,U0=n.d2*c*n.cc1/3,n.d3=(17*l0+_)*U0,n.d4=.5*U0*l0*c*(221*l0+31*_)*n.cc1,n.t3cof=n.d2+2*I,n.t4cof=.25*(3*n.d3+n.cc1*(12*n.d2+10*I)),n.t5cof=.2*(3*n.d4+12*n.cc1*n.d3+6*n.d2*n.d2+15*I*(2*n.d2+I)))}t3(n,0),n.init="n"}function E3(S,O){M3(S,{opsmode:O,satn:S.satnum,epoch:S.jdsatepoch-24332815e-1,xbstar:S.bstar,xecco:S.ecco,xargpo:S.argpo,xinclo:S.inclo,xmo:S.mo,xno:S.no,xnodeo:S.nodeo})}function o3(S,O){const o="i",t=S.substring(2,7),M=parseInt(S.substring(18,20),10),R=parseFloat(S.substring(20,32));let i=parseFloat(S.substring(33,43)),D=parseFloat(`${S.substring(44,45)}.${S.substring(45,50)}E${S.substring(50,52)}`);const d=parseFloat(`${S.substring(53,54)}.${S.substring(54,59)}E${S.substring(59,61)}`),E=parseFloat(O.substring(8,16))*C2,e=parseFloat(O.substring(17,25))*C2,l=parseFloat(`.${O.substring(26,33).replace(/\s/g,"0")}`),I=parseFloat(O.substring(34,42))*C2,W=parseFloat(O.substring(43,51))*C2,J=parseFloat(O.substring(52,63))/P2;i/=P2*1440,D/=P2*1440*1440;const g=M<57?M+2e3:M+1900,x=H2(g,R),{mon:Q,day:K,hr:H,minute:A,sec:U}=x,Z=V2(g,Q,K,H,A,U),u={error:0,satnum:t,epochyr:M,epochdays:R,ndot:i,nddot:D,bstar:d,inclo:E,nodeo:e,ecco:l,argpo:I,mo:W,no:J,jdsatepoch:Z};return E3(u,o),u}const L2=398600.4418,s3={stations:"ISS / CSS PARTNERS","indian-assets":"ISRO","cosmos-1408-debris":"CIS (ASAT DEBRIS)","iridium-33-debris":"US (DERELICT)","cosmos-2251-debris":"CIS (DERELICT)"},x3={"cosmos-1408-debris":"Cosmos 1408 — Russian ASAT test, 15 Nov 2021","iridium-33-debris":"Iridium 33 — collision with Cosmos 2251, 10 Feb 2009","cosmos-2251-debris":"Cosmos 2251 — collision with Iridium 33, 10 Feb 2009"};function C3(S){return/\bDEB\b/.test(S)?"DEBRIS":/\bR\/B\b/.test(S)?"ROCKET BODY":"PAYLOAD"}function e3(S){return S==="DEBRIS"?"SMALL":"LARGE"}function c3(S){const O=S.slice(9,17).trim(),o=Number(O.slice(0,2));return{intl:O,launchYear:o<57?2e3+o:1900+o}}function l3(S,O){var t;const o=[];for(const{group:M,text:R}of S){const i=R.split(/\r?\n/);for(let D=0;D+2<i.length;D+=3){const d=(t=i[D])==null?void 0:t.trim(),E=i[D+1],e=i[D+2];if(!d||!(E!=null&&E.startsWith("1 "))||!(e!=null&&e.startsWith("2 ")))continue;let l;try{l=o3(E,e)}catch{continue}if(l.error!==_0.None)continue;const I=l.no/60,W=Math.cbrt(L2/(I*I)),J=l.ecco,g=C3(d),{intl:x,launchYear:Q}=c3(E),K=n3(l.jdsatepoch).getTime();o.push({group:M,rec:l,object:{norad:Number(l.satnum),name:d,type:g,op:s3[M]??M.toUpperCase(),intl:x,launch:String(Q),alt:Math.round(W-u2),apogee:Math.round(W*(1+J)-u2),perigee:Math.round(W*(1-J)-u2),ecc:+J.toFixed(7),incl:+(l.inclo*180/Math.PI).toFixed(4),raan:+(l.nodeo*180/Math.PI).toFixed(4),argp:+(l.argpo*180/Math.PI).toFixed(4),ma:+(l.mo*180/Math.PI).toFixed(4),period:+(2*Math.PI*Math.sqrt(W*W*W/L2)/60).toFixed(2),rcs:e3(g),age:+((O-K)/864e5).toFixed(2),tle:[E,e]}})}}const h=new Set;return o.filter(M=>h.has(M.object.norad)?!1:(h.add(M.object.norad),!0))}const i3=`ISS (ZARYA)             
...
`,r3=W2,m3=Date.parse(W2.capturedAtUtc),h3=[{group:"stations",text:i3},{group:"indian-assets",text:d3},{group:"cosmos-1408-debris",text:I3},{group:"iridium-33-debris",text:A3},{group:"cosmos-2251-debris",text:a3}],N2=l3(h3,m3),F2=N2.map(S=>S.object),l2=new Map(N2.map(S=>[S.object.norad,S])),f3=S=>l2.get(S),g3=S=>{var O;return((O=l2.get(S))==null?void 0:O.object)??F2[0]},z3=S=>{var O;return((O=l2.get(S))==null?void 0:O.group)==="indian-assets"},u3=S=>{var O;return(O=l2.get(S))==null?void 0:O.group};F2.reduce((S,O)=>({...S,[O.type]:S[O.type]+1}),{PAYLOAD:0,"ROCKET BODY":0,DEBRIS:0});const T3=N2.reduce((S,O)=>({...S,[O.group]:(S[O.group]??0)+1}),{});F2.length;export{x3 as G,F2 as O,r3 as P,m3 as S,T3 as a,f3 as e,u3 as g,z3 as i,V2 as j,p3 as m,g3 as o,t3 as s};
