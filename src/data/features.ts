...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    status: 'partial',
    group: 'Operations',
    note: 'A TLE carries no covariance, so the 1-sigma the console uses for Pc is ASSUMED, not measured: it grows with real element-set age and with a radar cross-section class inferred from object type, since RCS lives in the SATCAT and not in a TLE. Both assumptions are disclosed on the detail view, they are declared COVARIANCE_METHOD = DEFAULT in every exported CDM, and the Thresholds screen exposes the sigma as a scale control that re-bands every event live, so you can see how much of the severity ranking rests on it. There is now also a MEASURED path, and it is honest about not being in use: `engine/tleUncertainty.ts` implements successive TLE differencing — propagate consecutive element sets for one object to their shared midpoint and take the RMS of the residuals in radial, along-track and cross-track — which is the published method (Flohrer et al., AMOS 2008) for getting a real uncertainty out of public data alone. It is tested against a known injected perturbation and recovers it to 1.00x. What it lacks is input: the committed snapshot is one instant per object by design, and this method needs several. `scripts/fetch-history.sh` collects the history and `npm run uncertainty` measures it. Until that has been run the console ranks on the modelled sigma, the measured table is empty, and the validation suite asserts that no measured covariance is claimed while it is.',
  },
  {
    id: 'conjunctions',
    label: 'Conjunctions',
