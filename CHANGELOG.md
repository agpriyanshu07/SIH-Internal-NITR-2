...
### PS-04 is now in the repository, mapped requirement by requirement

`PROBLEM.md` holds the statement verbatim — background, description, all four
expected outcomes, implementation notes — and a table putting each one against
the route, the registry entry and the status that answers it. Every expected
outcome is addressed.

Two of its lines had been guessed at until the statement arrived, and both
turned out to favour decisions already taken. The implementation notes say a 2D
orbit plot with flagged pairs is "a perfectly strong demo" and a 3D globe is
"not required to prove the concept", which makes the 2D viewer a match rather
than a shortfall. And "live/near-live" is the one line covered only in part: the
snapshot is one instant by choice, because screening element sets captured at
different times compares positions of differing trustworthiness and reports the
difference as geometry.

Registry status and requirement coverage are separate columns, because
conflating them produced a real contradiction in the first draft — `features.ts`
had orbital data as `live` while the same row read `partial`. Both were right:
the registry says whether a capability is real, coverage says how fully it
answers a requirement as worded.

### The presentation deck

`deck/KESSLER-PS04.pptx`, generated by `deck/build.cjs` so it can be rebuilt
when a figure changes rather than edited by hand. Seven slides on the SIH
structure: title, problem, solution, technical approach, feasibility, impact,
research. It uses the console's own palette and severity-chip motif, so the deck
and the demo read as one artefact.

Every number on it is one the repository reproduces. The impact slide takes one
real event — CARTOSAT-2C against a Cosmos 2251 fragment at 1.911 km — through to
the counterfactual: 3.16 km would drop it a band, 0.47x on sigma would too.

## Interoperability, measurement and triage

### Conjunction Data Messages, CCSDS 508.0-B-1

Everything computed here used to leave as a CSV that nothing downstream
understands. Every screened event now exports as a CDM, the format Space-Track
issues and ESA, NASA CARA and TraCSS consume. Every mandatory quantity was
already being computed, so this is a formatter that introduces no new number.

The covariance field is where emitting a standard format could turn an
assumption into a claim, and the standard had already solved it:
`COVARIANCE_METHOD` is `DEFAULT` on every object of every message, with a
COMMENT naming the model, the sigma, the element-set age and the RCS class. A
reader who checks that field learns the truth without reading this file.
Per-object sigmas recombine to the pair sigma in quadrature to 2.5e-16, so the
messages agree with the ranking they came from.

### Positional uncertainty, measured rather than assumed

`engine/tleUncertainty.ts` implements successive TLE differencing — propagate
two consecutive element sets to the midpoint of their epochs, difference the
states, RMS the residuals in radial, along-track and cross-track. It recovers a
known injected mean-motion perturbation to **1.00x**.

It is not in use, and the suite asserts that it is not: the committed snapshot
is one epoch per object, which is the one input the method cannot take.
`scripts/fetch-history.sh` collects a history and `npm run uncertainty` measures
it. Until then the console ranks on the modelled sigma and check 17 fails the
build if anything claims otherwise.

### The triage verdict

Every event now answers what would have to be true for it to stop mattering. The
miss distance that would drop it a band inverts in closed form. The sigma
multiplier that would do the same is found by sweeping rather than solving,
because Foster's Pc is **not** monotonic in sigma — it peaks near sigma =
miss/sqrt(2), and a bisection would find one root and silently ignore the other.
Real events land on both sides of that peak, and the panel explains the far side
when one does, because a larger sigma making an event less serious reads as a bug
otherwise.

### Scaling, measured instead of argued

`npm run scaling` runs the real cascade over six stride-sampled subsets of the
real catalogue. Fitted exponent **1.738** against 2.000 for pure all-pairs, and
throughput rises with n — 9,461 to 16,766 pairs per second — because the
per-pair setup amortises. Extrapolated single-threaded: 30,000 objects in about
170 minutes, with the limits of that extrapolation printed alongside it.

## Interface, after using it

### The console never scrolled the way it was built to

Every table here is written for a fixed-height screen, and none of it worked. A
grid item's automatic minimum size is its content, so two columns with no
`min-h-0` grew to 133,910px inside an `h-screen` grid and the document scrolled
instead of `<main>` — taking the sidebar, the top bar and the table's own header
off the top of the window. Three classes fixed it, and two things fell out that
had been invisible because the feature never ran: pinned headers were
transparent, and the dashboard table was clipping relative velocity and
collision probability entirely at 1440px.

### Three reported defects

Reading crammed into one side of the analysis and event pages: both are grids
where one column ends long before the other, so most of the scroll had a dead
column beside a narrow one. Both are now sticky, which also puts the controls
and the separation chart where they are useful.

Clicking Acknowledge moved every panel 104px down. Not the button — the
ACKNOWLEDGED chip is 101px wide and sits in a flex-wrap header, and between
1460px and 1540px those 101px pushed it from one line to two. The chip now
renders in both states and is `invisible` when it does not apply.

### The type scale was doing two jobs

The artboards were a dashboard, where 9-12px is right. The analysis screens are
several hundred words of argument, and they inherited `base` at 13px with their
explanatory paragraphs at 10px — table-header size. The scale is split at the
seam: `sm+` and below unchanged, `base` and up moved to reading sizes, and 25
prose paragraphs moved with them, to 15px in wide panels and 12px in the 340px
control rail.
