...
`deck/KESSLER-PS04.pptx` is the SIH-style presentation, generated by
`deck/build.cjs` rather than hand-edited so it can be rebuilt when a figure
changes:

```bash
node deck/build.cjs      # needs pptxgenjs
```

Seven slides on the standard structure — title, problem, solution, technical
approach, feasibility, impact, research — in the console's own palette, so the
deck and the demo read as one thing. Every number on it is one a command in this
repository reproduces.

The third deliverable, a 3-minute video, does not exist yet. `DEMO.md` section 2
